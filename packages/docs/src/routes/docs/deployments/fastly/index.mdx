...
The file simply re-exports the produced `fetch` handler and attaches the fastly fetch event listener as shown below.

```tsx title="/dist/index.js"
import { fetch } from "../server/entry.fastly"; export default { fetch };
